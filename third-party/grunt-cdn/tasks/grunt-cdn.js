...
module.exports = function (grunt) {
	...
	function cdnUrl(resource, filename, relativeTo) {
		...
		if (!options.flatten && !grunt.file.isPathAbsolute(resourceUrl.pathname)) {
			// console.log('\n\n\n\n\n resource:'+resource + ', filepath:'+ path.resolve( path.dirname('/'+filename),'../bg.png') +'\n\n\n\n\n');
			// console.log(options.fuck+'\n\n\n\n');
			console.log('absolute: ' + path.resolve(path.dirname(filename), resource));
			console.log('relative: ' + path.relative(options.fuck, path.resolve(path.dirname(filename), resource)));
			resource = path.relative(options.fuck, path.resolve(path.dirname(filename), resource));
		...
		console.log('relativeTo 4: ' + relativeTo + ', resource: '+resource );

		// var src = path.join(relativeTo, resourceUrl.pathname).replace(/\\/g, '/').replace(/:\/(\w)/, '://$1');
		var src = path.join(relativeTo, resource).replace(/\\/g, '/').replace(/:\/(\w)/, '://$1');
