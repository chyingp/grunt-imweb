    ...
    files.forEach(function (file) {
      ...
      proc.blocks.forEach(function (block) {
        ...
        // updated it on data-main attribute
          ...
          // casper
          grunt.log.subhead('requirejs.options is now:')
            .subhead('  requirejs.options:')
            .writeln('  ' + inspect(requirejs.options));

        if (block.requirejs) {

          // log a bit what was added to config
          grunt.log.subhead(file.path+' is now:')
            .subhead('  requirejs:')
            .writeln('  ' + inspect(block.requirejs));
          ...
          var task = requirejs[file.path] = {options:{}};
          var opt = task.options;
          var options = requirejs.options || {};

          if (options) {
            opt.name = options.name || block.requirejs.name;
            opt.out = options.out || block.requirejs.dest;
            opt.baseUrl = options.baseUrl || block.requirejs.baseUrl;
            opt.mainConfigFile = options.mainConfigFile || path.join(opt.baseUrl, opt.name) + '.js';
          } else {
            task.options = {
              name: block.requirejs.name,
              out: block.requirejs.dest,
