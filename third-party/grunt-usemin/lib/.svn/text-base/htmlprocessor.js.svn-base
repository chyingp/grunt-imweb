    ...
    if (block && last) {
      ...
      if (asset && asset[2]) {
        ...
        if (main) {
          ...
          if(!last.requirejs.origScript.match(/^https?:\/\//)){
...
HTMLProcessor.prototype.replaceWith = function replaceWith(block) {
  ...
  if (block.type === 'css') {
    ...
    var requireSrc = block.requirejs.srcDest;
    if(!block.requirejs.srcDest.match(/^https?:\/\//)){
      requireSrc = path.relative(this.relativeSrc, block.requirejs.srcDest);  
    }
